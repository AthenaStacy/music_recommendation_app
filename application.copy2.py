...
def next2(): #remember the function name does not need to match th eURL
	if request.method == 'GET':
		...
		f = open(filename,'a') #a is for append
		f.write('%s\n'%(app.currentq2))
		f.write('%s\n\n'%(request.form['answer_from_layout2']))
		f.write('%s\n'%(app.vars['song_name']))
...
def pickasong(): #remember the function name does not need to match th eURL
	if request.method == 'GET':
		...
		print "song number is", request.form['song_pick']
		print "picked song is ", app.vars['song_name']
 	return redirect('/main')
...
def disambiguate(): #remember the function name does not need to match th eURL
	if request.method == 'GET':
		...
		print "song number is", request.form['artist_pick']
		print "picked artist is ", app.vars['artist_name']
 	return redirect('/main')
...
def song_check():
	...
	print('Searching for ', app.vars['song_or_artist'])

	if len(match)==1 and app.vars['song_or_artist'] == 'Song':
		app.vars['artist_name'] = lyrics_df[lyrics_df['track_name']==song_i_like].artist_name.values[0]
		return(1)

	print 'number of matches = ', len(match)
	if len(match)==0:
		print ('Could not find that song or artist!  Please go back and try again.')
		return(2)
    ##################################################################################

	if(app.vars['song_or_artist'] == 'Artist'):
		app.song_list={}
		print 'Looking for songs to match that artist'
		song_list = lyrics_df[lyrics_df['artist_name']==artist_i_like]
		print song_list['track_name']
		song_list = song_list.reset_index()
		for x in range(len(song_list['track_name'])):
			print 'song_list[x] =', song_list['track_name'][x]
			app.song_list[x] = song_list['track_name'][x]
		return(3)

	if len(match)>0 and app.vars['song_or_artist'] == 'Song':
		app.match_artist = {}
		app.match_song = {}
		app.match_date = {}
		print ('Need to disambiguate song...')
		match = lyrics_df[lyrics_df['track_name']==song_i_like]
...
def find_similar():
	...
	#record artist for the inputted song
	#app.vars['artist_name'] = lyrics_df[lyrics_df['track_name']==song_i_like].artist_name.values[0]
	...
	sentiment = lyrics_df[lyrics_df['track_name']==song_i_like].lex_diversity.values[0]
	lyrics_df['dist'] = lyrics_df.lex_diversity.map((lambda x: abs(sentiment - x)))	
	...
	result = lyrics_df[lyrics_df['date']>datetime.date(2013,1,1)].sort_values('dist')[:nreturn] 
 	...
 	for x in range(0,nreturn):
 		...
 		app.lyricsim[x] = result['date'][x]
 		app.sentiment[x] =   result['date'][x]
